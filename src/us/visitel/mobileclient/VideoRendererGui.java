...
//    { SCALE_ASPECT_FIT, SCALE_ASPECT_FILL, SCALE_FILL };
    ...
    private static class YuvImageRenderer implements VideoRenderer.Callbacks {
        ...
        private void checkAdjustTextureCoords0() {
...
//                    Log.d(TAG, "  Texture UV offsets: " + texOffsetU + ", " + texOffsetV);
        ...
        private void checkAdjustTextureCoords1() {
            if (!updateTextureProperties) {
                return;
            }
            synchronized(updateTextureLock) {
...
//                        " x " + videoHeight);
                if (displayWidth > 1 && displayHeight > 1 &&
                    ...
                    if (scalingType == ScalingType.SCALE_ASPECT_FIT) {
                        // Need to re-adjust vertices width or height to match video AR.
                        if (displayAspectRatio > videoAspectRatio) {
                            float deltaX = (displayWidth - displayHeight * videoAspectRatio) /
                                    instance.screenWidth;
                            texRight -= deltaX;
                            texLeft += deltaX;
                        } else {
        ...
        private void draw() {
            ...
            if (id == 0) {
                checkAdjustTextureCoords0();
            } else if (id == 1) {
                checkAdjustTextureCoords1();
            }
...
//                        Log.d(TAG, "id: " + id + " angle: " + frameFromQueue.rotationDegree);
                        if (frameFromQueue.rotationDegree != rotationDegree) {
                            rotationDegree = frameFromQueue.rotationDegree;
...
//                return;
                ...
                yuvPlanes[0] = ByteBuffer.allocateDirect(frame.width*frame.height);
                yuvPlanes[1] = ByteBuffer.allocateDirect(frame.width*frame.height/4);
                yuvPlanes[2] = ByteBuffer.allocateDirect(frame.width*frame.height/4);
                yuvFrameToRender = new I420Frame(frame.width, frame.height, frame.rotationDegree, frame.yuvStrides, yuvPlanes, 0);
                //textureFrameToRender = new I420Frame(frame.width, frame.height, frame.rotationDegree, -1);
                updateTextureProperties = true;
            } else if (videoWidth != frame.width || videoHeight != frame.height) {
                videoWidth = frame.width;
                videoHeight = frame.height;
                ByteBuffer[] yuvPlanes = new ByteBuffer[3];
                yuvPlanes[0] = ByteBuffer.allocateDirect(frame.width*frame.height);
            ...
            if (frame.yuvFrame) {
                ...
                VideoRenderer.nativeCopyPlane(frame.yuvPlanes[0], frame.width, frame.height,
                        frame.yuvStrides[0], yuvFrameToRender.yuvPlanes[0],
                        yuvFrameToRender.yuvStrides[0]);
                VideoRenderer.nativeCopyPlane(frame.yuvPlanes[1], frame.width/2, frame.height/2,
                        frame.yuvStrides[1], yuvFrameToRender.yuvPlanes[1],
                        yuvFrameToRender.yuvStrides[1]);
                VideoRenderer.nativeCopyPlane(frame.yuvPlanes[2], frame.width/2, frame.height/2,
                        frame.yuvStrides[2], yuvFrameToRender.yuvPlanes[2],
                        yuvFrameToRender.yuvStrides[2]);
                yuvFrameToRender.rotationDegree = frame.rotationDegree;
                rendererType = RendererType.RENDERER_YUV;
                frameToRenderQueue.offer(yuvFrameToRender);
            } else {
                textureFrameToRender = new I420Frame(frame.width, frame.height, frame.rotationDegree, frame.textureId, frame.samplingMatrix, 0);
