 ...
 */
public class PeerConnectionClientCamera1 {
  ...
  private static final PeerConnectionClientCamera1 instance = new PeerConnectionClientCamera1();
  ...
  private PeerConnectionClientCamera1() {
  ...
  public static PeerConnectionClientCamera1 getInstance() {
    return instance;
  }
  ...
  public void createPeerConnection(final android.opengl.EGLContext renderEGLContext,
  ...
  private void createMediaConstraintsInternal() {
    ...
    if (videoCallEnabled) {
      ...
      pcConstraints.mandatory.add(new MediaConstraints.KeyValuePair(
              MAX_VIDEO_WIDTH_CONSTRAINT, Integer.toString(MAX_VIDEO_WIDTH)));
      pcConstraints.mandatory.add(new MediaConstraints.KeyValuePair(
              MIN_VIDEO_HEIGHT_CONSTRAINT, Integer.toString(MIN_VIDEO_HEIGHT)));
      pcConstraints.mandatory.add(new MediaConstraints.KeyValuePair(
              MAX_VIDEO_HEIGHT_CONSTRAINT, Integer.toString(MAX_VIDEO_HEIGHT)));
    }
  ...
  private void createPeerConnectionInternal(android.opengl.EGLContext renderEGLContext) {
    ...
    if (dataChannelEnabled) {
      ...
      dataChannel = peerConnection.createDataChannel("VisiTel data", init);
    }
    isInitiator = false;
    ...
    mediaStream = factory.createLocalMediaStream("VTAMS");
    if (videoCallEnabled) {
  ...
  public void createOfferWithOptions(final PeerConnection.OfferAnswerOptions options) {
    executor.execute(new Runnable() {
      @Override
      public void run() {
        if (peerConnection != null && !isError) {
          Log.d(TAG, "PC Create OFFER");
          isInitiator = true;
          peerConnection.createOfferWithOptions(sdpObserver, sdpMediaConstraints, options);
        }
      }
    });
  }
  ...
  public void restartICE() {
    PeerConnection.OfferAnswerOptions options = new PeerConnection.OfferAnswerOptions();
    options.ice_restart = true;
    createOfferWithOptions(options);
  }
      ...
      public void run() {
        ...
        if (peerConnection != null && !isError) {
          if (queuedRemoteCandidates != null) {
            Log.d(TAG, "add queuedRemoteCandidates");
        ...
        public void run() {
          ...
          if (stream.videoTracks.size() == 1) {
            ...
            for (VideoRenderer.Callbacks remoteRender : remoteRenders) {
              Log.d(TAG, "remoteRenderer: " + remoteRender);
              remoteVideoTrack.addRenderer(new VideoRenderer(remoteRender));
            }
            events.onStreamAdded();
          }
        }
      });
    }
