...
import com.combinedpublic.mobileclient.Classes.CustomCamera2CapturerListener;
import com.combinedpublic.mobileclient.Classes.CustomCameraCapturerListener;
...
public class CameraCapturerCompat implements VideoCapturer {
    ...
    private CustomCameraCapturerListener listener;
    private CustomCamera2CapturerListener listener2;

    private String LOG_TAG = "CameraCapturerCompat";
    ...
    public CameraCapturerCompat(Context context, Source cameraSource) {
        if (Camera2Capturer.isSupported(context) && isLollipopApiSupported()) {
            ...
            listener2 = new CustomCamera2CapturerListener();
            camera2Capturer = new Camera2Capturer(context, camera2IdMap.get(cameraSource), listener2);
            activeCapturer = camera2Capturer;
            camera1Capturer = null;
        } else {
            setCamera1Maps();
            listener = new CustomCameraCapturerListener();
            camera1Capturer = new CameraCapturer(context, camera1IdMap.get(cameraSource), listener);
    ...
    public void switchCamera() {
        ...
        if (usingCamera1()) {
            ...
            if (camera1Capturer != null) {
                Log.d(LOG_TAG,"(TWILIO) camera1Capturer switchCamera is called");
                camera1Capturer.switchCamera(newCameraId); // newCameraId
            }
        } else {
            Log.d(LOG_TAG,"(TWILIO) camera2Capturer NewCameraId is " + newCameraId);
            if (camera2Capturer != null) {
                Log.d(LOG_TAG,"(TWILIO) camera2Capturer switchCamera is called");
                camera2Capturer.switchCamera(newCameraId); // newCameraId
            }
        }
    }
